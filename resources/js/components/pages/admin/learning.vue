...
const search = async () => {
    let response = await axios.get(
        "/api/search_learning?s=" + searchLearning.value
    );

    learnings.value = response.data.learning;

    console.log("search", response.data.learning);
};
const createNewLearning = async () => {
    await axios.post("/api/create_learning", form.value).then((response) => {
        getLearnings();
        form.value = {};
...
const updateLearning = async () => {
    await axios
        .post("/api/update_learning/" + form.value.id, form.value)
        .then(() => {
            getLearnings();
            form.value = {};
            editMode.value = false;
...
const deleteLearning = (id) => {
    Swal.fire({
        ...
        if (result.value) {
            axios.get("/api/delete_learning/" + id).then(() => {
                Swal.fire("Delete", "Learning delete successfully", "success");
                getLearnings();
            });
        }
    });
                                ...
                                >
                                    ...
                                    <div class="col-md-8">
                                        <form class="rounded position-relative">
                                            <input
                                                 v-model="searchLearning" @keyup="search()"
